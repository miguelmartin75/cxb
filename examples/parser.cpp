...
SourceLoc lex_loc(Parser* ctx);
Token lex_next(Parser* ctx);
...
#define LHS(x) ((x).kids.data[0])
#define RHS(x) ((x).kids.data[1])
#define ADD_KID(x, k) push_back(ctx->tree, (x)->kids, k)

#define ADD_ERR(node, err_msg, ...) { \
    node->err = 1; \
    push_back(ctx->error_arena, array_slice_from_pod<ParseError>(ctx->errors), ParseError{ \
        (node), \
        err_msg \
    }) \
}
...
#define CHECK(x, err_msg) if(!(x)) { ADD_ERR_NO_NODE(err_msg); }
#define CHECK_WN(x, err_msg, node) if(!(x)) { ADD_ERR(node, err_msg); }
#define EXPECT(tok, expected) CHECK((tok).kind == (expected), S8_LIT(STR_CONCAT("expected: ", TO_STR(x))))
#define EXPECT_WN(tok, expected, node) CHECK_WN((tok).kind == (expected), (S8_LIT("unexpected token - TODO list expected token")), node)
...
static inline AstNode* add_node(
    Parser* ctx,
    NodeKind kind,
    AstNodeData data,
    Token tok = Token{},
...
static AstNode* parse_expression_base(Parser* ctx) {
    ...
    if(next_token.kind == TOK_BRACKET_LEFT) {
        ...
        // TODO: parse float
        Token lit = NEXT;
        auto literal_token = lit.sv(ctx->buffer);
        ...
        NODE(node).data.numeral_literal.value = lit.sv(ctx->buffer)[0] == 't';
        return node;
        ...
        Token lit = NEXT;
        auto int_literal_token = lit.sv(ctx->buffer);
        AstNode* node = add_node(ctx, NODE_INT_LIT, {}, lit);
        if(!lit.err) {
        ...
        case NODE_UNION_DECL: {
            ...
            while(PEEK.kind != TOK_SCOPE_BRACKET_RIGHT) {
                ...
                if(PEEK.kind == TOK_EQUALS_OP) {
                    ...
                    ADD_KID(field, INVALID_NODE_IDX);
                }
...
static AstNode* parse_var_decl(Parser* ctx) {
    ...
    // kids[0] = type or INVALID_NODE_IDX
    // kids[1] = assignment or INVALID_NODE_IDX
    Token tok = NEXT;
    Token name = tok;
...
#define CONSUME ctx->idx += 1; ctx->loc.col += 1
#define CONSUME_X(x) ctx->idx += x; ctx->loc.col += x
#define CONSUME_NEXT_LINE ctx->idx += 1; ctx->loc.line += 1; ctx->loc.col = 0
