...
int main(void) {
    String8 p1 = {.data = "foo", .len = 3, .null_term = 1};
    String8 p2 = {.data = "bar", .len = 3, .null_term = 1};
    ...
    String8 res_slice = {.data = result.data, .len = result.len, .null_term = result.null_term};

    String8 slice_foo = cxb_ss_slice(res_slice, 0, 1);
    ASSERT(cxb_ss_size(slice_foo) == 2);
    ASSERT(!cxb_ss_empty(slice_foo));

    String8 slice_bar = cxb_ss_slice(res_slice, 3, -1);
